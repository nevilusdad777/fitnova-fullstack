...
import { LucideAngularModule, Target, Trophy, TrendingUp, TrendingDown, Save, Activity, Calendar } from 'lucide-angular';
...
export class GoalsComponent implements OnInit {
  ...
  paces = [
    { id: 'relaxed', label: 'Relaxed (0.25kg/wk)', adjustment: { loss: -250, gain: 150 }, rate: 0.25 },
    { id: 'normal', label: 'Normal (0.5kg/wk)', adjustment: { loss: -500, gain: 300 }, rate: 0.5 },
    { id: 'aggressive', label: 'Aggressive (1kg/wk)', adjustment: { loss: -1000, gain: 500 }, rate: 1.0 }
  ];

  constructor() {
    this.goalsForm = this.fb.group({
      mainGoal: ['loss', Validators.required],
      ...
      pace: ['normal', Validators.required]
    });
  }

  caloricAdjustment(): string {
    const goal = this.goalsForm.get('mainGoal')?.value as 'loss' | 'gain' | 'maintain';
    ...
    const adj = goal === 'loss' ? pace.adjustment.loss : pace.adjustment.gain;
    const sign = adj > 0 ? '+' : '';
  ...
  estimatedDate(): string {
    ...
    if (goal === 'loss' && target >= current) return 'Check Target Weight';
